...
void declare_var(Environment *env, const char *varname, RuntimeVal *value) {    
    ...
    while (env->entries[index].key != NULL) {
        if (strcmp(env->entries[index].key, varname) == 0) {
            ...
            snprintf(error_message, sizeof(error_message), "Cannot declare variable %s. It is already defined.\n", varname);
            error(error_message);
        }
...
RuntimeVal *lookup_var(Environment *env, const char *varname) {
    ...
    char error_message[100];
    snprintf(error_message, sizeof(error_message), "Value not found for variable %s\n", varname);
    error(error_message);
}
...
Environment *resolve(Environment *env, const char *varname) {
    ...
    char error_message[100];
    snprintf(error_message, sizeof(error_message), "Cannot resolve variable '%s' as it does not exist.", varname);
    error(error_message);
