...
NilVal *MK_NIL() {
    NilVal *val = (NilVal *)malloc_safe(
        sizeof(NilVal), "NilVal"
    );
    val->base.type = NIL_T;
    return val;
}
...
BooleanVal *MK_BOOL(unsigned short int b) {
    BooleanVal *val = (BooleanVal *)malloc_safe(
        sizeof(BooleanVal), "BooleanVal"
    );
...
NumberVal *MK_NUMBER(double n) {
    NumberVal *val = (NumberVal *)malloc_safe(
        sizeof(NumberVal), "NumberVal"
    );
...
FunctionVal *MK_FUNCTION(char **params, size_t param_count, Stmt **body, size_t body_count, Environment *env, RuntimeVal *(*builtin_func)(Environment *env, RuntimeVal **args, size_t arg_count)) {
...
TableVal *MK_TABLE(char **columns, size_t column_count) {
    ...
    table->columns = (char **)malloc_safe(sizeof(char *) * column_count, "TableVal columns");
...
RuntimeVal *create_native_fn(char **params, size_t param_count, RuntimeVal *(*fn)(Environment *env, RuntimeVal **args, size_t arg_count)) {
