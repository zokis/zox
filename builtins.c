...
void register_builtins(Environment *env) {
  ...
  declare_var(env, "keys",
              (RuntimeVal *)MK_FUNCTION(keys_params, keys_param_count, NULL, 0,
                                        env, builtin_keys));

  char *len_params[] = {"list_or_dict"};
  size_t len_param_count = 1;
  declare_var(env, "len",
              (RuntimeVal *)MK_FUNCTION(len_params, len_param_count, NULL, 0,
                                        env, builtin_len));

  char *print_params[] = {"value"};
  size_t print_param_count = 1;
  declare_var(env, "print",
              (RuntimeVal *)MK_FUNCTION(print_params, print_param_count, NULL,
                                        0, env, builtin_print_value));

  char *println_params[] = {"value"};
  size_t println_param_count = 1;
  declare_var(env, "println",
              (RuntimeVal *)MK_FUNCTION(println_params, println_param_count,
                                        NULL, 0, env, builtin_println_value));

  char *random_params[] = {};
  size_t random_param_count = 0;
  declare_var(env, "random",
              (RuntimeVal *)MK_FUNCTION(random_params, random_param_count, NULL,
                                        0, env, builtin_random));

  char *random_int_params[] = {"min", "max"};
  size_t random_int_param_count = 2;
  declare_var(
      env, "random_int",
      (RuntimeVal *)MK_FUNCTION(random_int_params, random_int_param_count, NULL,
                                0, env, builtin_random_int));

  char *values_params[] = {"dict"};
  size_t values_param_count = 1;
  declare_var(env, "values",
              (RuntimeVal *)MK_FUNCTION(values_params, values_param_count, NULL,
                                        0, env, builtin_values));

  char *find_params[] = {"list_or_string", "value"};
  size_t find_param_count = 2;
  declare_var(env, "find",
              (RuntimeVal *)MK_FUNCTION(find_params, find_param_count, NULL, 0,
                                        env, builtin_find));
}
