...
NilLiteral *create_nil_literal() {
    return &preallocated_nil_literal;
}
...
TableLiteral *create_table_literal(char **columns, size_t column_count) {
    TableLiteral *table = (TableLiteral *)malloc_safe(sizeof(TableLiteral), "TableLiteral");
...
void free_expr(Expr *expr) {
    ...
    switch (expr->stmt.kind) {
        ...
        case BooleanLiteralAst: {
            if (expr == (Expr *)&preallocated_true_literal || expr == (Expr *)&preallocated_false_literal) {
                return;
            }
            free_safe(expr);
            break;
